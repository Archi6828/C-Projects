...
TEST(SampleTests, Size)
{
    MyPriorityQueue<int> mpq;
    mpq.insert(3);
    EXPECT_TRUE(mpq.size() == 1);

    MyPriorityQueue<int> mpq1;
    EXPECT_TRUE(mpq1.size() == 0);

    MyPriorityQueue<int> mpq2;
    mpq2.insert(3);
    mpq2.insert(5);
    mpq2.insert(3);
    EXPECT_TRUE(mpq2.size() == 3);
}

TEST(SampleTests, EmptyPQ)
{
    MyPriorityQueue<int> mpq;
...
TEST(SampleTests, ExcepPQ)
{
    ...
    EXPECT_THROW({ 
        ...
        {
            EXPECT_TRUE("Min Heap is Empty!!" == e.getMessage());
            throw;
        }
...
TEST(SampleTests, ExcepExtractMInPQ)
{
    MyPriorityQueue<int> mpq;
    EXPECT_THROW({
        try
...
TEST(SampleTests, ExtractMin)
{
    ...
    std::vector<char> v1 = {'C','H','E','M','I','S','T','R','Y'};
    EXPECT_TRUE(mpq.MinHeap() == v1);

    mpq.extractMin();
    std::vector<char> v = {'E', 'H', 'S', 'M', 'I', 'Y' , 'T', 'R'};
    EXPECT_TRUE(mpq.MinHeap() == v);
}

TEST(SampleTests, MST1)
{
